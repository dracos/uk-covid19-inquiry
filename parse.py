...
def parse_transcript(url, text):
    ...
    for page in data.keys():
        ...
        for num, line in data[page]:
            ...
            # Date at start
            m = re.match(r' *(Mon|Tues|Wednes|Thurs|Fri)day,? \d+(nd|th)? (August|September|October|November|December|January|February|March|April|May|June|July) 20[02][1-5]\.?$', line)
            if m:
